...
if __name__ == '__main__':
    ...
    for i in range(num_features):
        ...
        for features in X:
            if len(features) == num_features:
                sum_feature += features[i]
            
        for features in X:
            if len(features) < num_features:
                features.append(sum_feature/len(leaves))
    ...
    for epoch in range(EPOCHS):
        ...
        for step, idx_batch in enumerate(tqdm(train_batch_gen)):
            ...
            for i in range(len(y_hat)):
                y_hat2.append([])
                y_hat2[i].append(1 - float(y_hat[i]))
                y_hat2[i].append(float(y_hat[i]))
            y_hat2 = torch.tensor(y_hat2, dtype=torch.double, device=device, requires_grad=True)
            #print(y_hat2)
            #print(y[idx_batch])
            ...
            train_loss = loss_function(y_hat2, y[idx_batch].type(torch.LongTensor))
    ...
    with torch.no_grad():
        ...
        for i in range(len(y_hat)):
            ...
            y_hat2[i].append(float(y_hat[i]))
        y_hat2 = torch.tensor(y_hat2, dtype=torch.double, device=device, requires_grad=True)
        loss = loss_function(y_hat2, y[test_idx].type(torch.LongTensor))
        delta_loss = dendronet.delta_loss()
