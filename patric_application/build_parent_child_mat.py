...
"""
def build_pc_mat(genome_file='genome_lineage.csv', label_file='erythromycin_firmicutes_samples.csv', leaf_level='genome_id', all_genomes=False, force_build=False, save_matrix=True, new_method=False):
    ...
    if save_matrix:
        ...
        jdict['leaves'] = leaves
        with open(os.path.join('data_files', 'parent_child_matrices', antibiotic + '_' + group + '_(' + leaf_level + ').json'), 'w') as outfile:
