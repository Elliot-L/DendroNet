...
"""
def build_pc_mat(genome_file='genome_lineage.csv', label_file='erythromycin_firmicutes_samples.csv', force_build=False):
    ...
    jdict['nodes'] = nodes
    jdict['leaves'] = leaves
    with open(os.path.join('data_files', 'parent_child_matrices', antibiotic + '_' + group + '.json'), 'w') as outfile:
...
def build_pc_mat_for_all_species(genome_file='genome_lineage.csv', set_level='phylum', name='Firmicutes'):
    genome_df = pd.read_csv(genome_file, delimiter='\t', dtype=str)
    genome_df = genome_df.rename(columns={'class': 'safe_class'})
    genome_df = genome_df[genome_df[set_level] == name]
    genome_df = genome_df[
